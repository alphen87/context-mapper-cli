...
public class SketchMinerLinkGenerator extends AbstractContextMappingModelGenerator {

    private static final String SKETCH_MINER_FILE_EXT = "sketch_miner_url";
    ...
    protected void generateFromContextMappingModel(ContextMappingModel model, IFileSystemAccess2 fsa, URI inputFileURI) {
        ...
        for (BoundedContext boundedContext : model.getBoundedContexts()) {
            for (Flow flow : getFlowsWithSteps(boundedContext)) {
                ...
                fsa.generateFile(fileName + "_BC_" + boundedContext.getName() + "_" + flow.getName() + "." + SKETCH_MINER_FILE_EXT, link);
            }
        }
    }
